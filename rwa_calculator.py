...
class RWACalculator:
    ...
    def _apply_provision_weighting(self, row):
        ...
        provision = row.get('provision_constitue', 0)
        montant = row.get('montant', 0)
        
        if montant == 0:
            return 1.5
        
        # Calcul du taux de provisionnement
        provision_rate = provision / montant
        
        # Application des règles de pondération selon le provisionnement
        if provision_rate >= 0.2:  # 20% ou plus de provisions
            return max(0, (montant - provision) / montant * 1.5)
        else:
            return 1.5  # Pondération maximale si pas assez provisionné
    
    def _get_sovereign_weighting(self, row):
        """Méthode héritée - calcul pondération souveraine"""
        return self._calculate_sovereign_weighting(row)
    
    def _get_public_org_weighting(self, row):
        """Méthode héritée - calcul pondération organisme public"""
        return self._calculate_public_org_weighting(row)
    
    def _get_bmd_weighting(self, row):
        """Méthode héritée - calcul pondération BMD"""
        return self._calculate_bmd_weighting(row)
    
    def _get_credit_institution_weighting(self, row):
        """Méthode héritée - calcul pondération établissement crédit"""
        return self._calculate_credit_institution_weighting(row)
    
    def _get_enterprise_weighting(self, row):
        """Méthode héritée - calcul pondération entreprise"""
        return self._calculate_enterprise_weighting(row)
    
    def _get_short_term_weighting(self, row):
        """Méthode héritée - calcul pondération court terme"""
        note_externe_raw = row.get('note_externe', '')
        note_externe = str(note_externe_raw).upper() if pd.notna(note_externe_raw) else 'DEFAULT'
        return self._get_short_term_rating_weighting(note_externe)
